...
public interface OrderItemRepository extends JpaRepository<OrderItemEntity, UUID> {
    ...
    @Query("SELECT o FROM OrderItemEntity o WHERE o.cart.lastModifiedAt BETWEEN :start AND :end " +
            "AND o.deleted = false " +
            "AND o.cart.cartStatus != 'OPEN' " +
            "AND o.cart.status IS NOT NULL")
    List<OrderItemEntity> findAllByLastModifiedAtBetween(@Param("start") Instant start, @Param("end") Instant end);
}
